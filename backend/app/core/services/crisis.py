from __future__ import annotations

from app.core.domain.enums import BeliefSource, EventKind, Severity, TruthRel
from app.core.domain.models import Belief, GameSession, WorldEvent


class CrisisTick:
    """
    通用危机 tick：读 world_flags 键，不写死落霞剧情名。

    - countdown==0 且有 blood_curse_planted 且 **未** disarmed 且未 crisis_fired
      → 护山危局
    - countdown==0 且 **已** disarmed（或 sect_stabilized）且未 crisis_averted_noted
      → 有为成功：危而不溃的安稳事件（证明玩家改了因果）
    """

    def run(self, session: GameSession) -> list[WorldEvent]:
        flags = session.world_flags or {}
        events: list[WorldEvent] = []

        try:
            cd = int(flags.get("xuanyin_countdown", -1))
        except (TypeError, ValueError):
            cd = -1

        disarmed = bool(flags.get("blood_curse_disarmed") or flags.get("sect_stabilized"))
        planted = bool(flags.get("blood_curse_planted"))

        if cd == 0 and planted and not disarmed and not flags.get("crisis_fired"):
            session.world_flags["crisis_fired"] = True
            session.world_flags["sect_at_brink"] = True
            prop = "天地异象大作，护山阵剧烈震颤，恐有大祸"
            self._broadcast_belief(session, prop, TruthRel.MATCHES_AUTHORITY)
            known = [a for a, st in session.states.items() if st.alive]
            ev = WorldEvent(
                kind=EventKind.WORLD,
                severity=Severity.CRITICAL,
                title="护山异变",
                summary=prop,
                actor_ids=[],
                location=None,
                day=session.day,
                known_to=known,
                card_headline="大劫将至",
                card_body=prop + "。有人仍以为是「秘境开启」之兆。",
                involves_player=session.player_id() in known,
                tags=["crisis", "countdown_zero", "tragedy_gravity"],
            )
            session.events.append(ev)
            events.append(ev)

        elif cd == 0 and planted and disarmed and not flags.get("crisis_averted_noted"):
            # 玩家有为：同一期限节点，结果不同
            session.world_flags["crisis_averted_noted"] = True
            session.world_flags.pop("sect_at_brink", None)
            prop = "开启之日已至，山门震动片刻，却被先手镇压，化险为夷"
            self._broadcast_belief(session, prop, TruthRel.MATCHES_AUTHORITY)
            known = [a for a, st in session.states.items() if st.alive]
            ev = WorldEvent(
                kind=EventKind.WORLD,
                severity=Severity.MAJOR,
                title="化险为夷",
                summary=prop,
                actor_ids=[session.player_id()],
                day=session.day,
                known_to=known,
                card_headline="劫数改写",
                card_body=prop + "。若无人先手，此日或是宗门末日。",
                involves_player=True,
                tags=["crisis_averted", "countdown_zero", "player_agency"],
            )
            session.events.append(ev)
            events.append(ev)

        # 倒计时提醒（10/3 日）——已解咒时文案略不同
        if cd in (10, 3) and not flags.get(f"countdown_ping_{cd}"):
            session.world_flags[f"countdown_ping_{cd}"] = True
            if disarmed:
                prop = f"距传闻「开启之日」约 {cd} 天；你已知阵上有备，心中稍定"
            else:
                prop = f"距传闻中的「开启之日」大约还有 {cd} 天"
            pid = session.player_id()
            session.beliefs.setdefault(pid, []).append(
                Belief(
                    belief_id=f"countdown_ping_{cd}",
                    holder_id=pid,
                    proposition=prop,
                    source=BeliefSource.RUMOR,
                    truth_rel=TruthRel.UNKNOWN_TO_AUTHORITY,
                    confidence=0.6,
                    day=session.day,
                )
            )
            ev = WorldEvent(
                kind=EventKind.WORLD,
                severity=Severity.MINOR if cd == 10 else Severity.MAJOR,
                title="期限将近",
                summary=prop,
                actor_ids=[pid],
                day=session.day,
                known_to=[pid],
                card_headline="时限",
                card_body=prop,
                involves_player=True,
                tags=["countdown_ping"],
            )
            session.events.append(ev)
            events.append(ev)

        return events

    def _broadcast_belief(
        self, session: GameSession, prop: str, truth: TruthRel
    ) -> None:
        for aid, st in session.states.items():
            if not st.alive:
                continue
            session.beliefs.setdefault(aid, []).append(
                Belief(
                    belief_id=f"crisis_d{session.day}_{aid}",
                    holder_id=aid,
                    proposition=prop,
                    source=BeliefSource.WITNESS,
                    truth_rel=truth,
                    confidence=0.85,
                    day=session.day,
                )
            )
